...
-- 思路
-- 1. 先筛选出invoice > 20 的记录,同时将时间转化，作为temp表
select
    date_format(order_date, '%Y-%m') as month, order_id, customer_id,invoice
from
    Orders
where
    invocie > 20

-- 2. 从筛选出的结果重新选出符合条件的结果
select
    month, count(order_id), count(distinct customer_id) as customer_count
from
    (select
    date_format(order_date, '%Y-%m') as month, order_id, customer_id,invoice
from
